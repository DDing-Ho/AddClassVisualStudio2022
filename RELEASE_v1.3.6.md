...
- `JoinableTaskFactory` 계열 호출을 제거해서, 일부 로컬 환경에서 `VsPackage`가 로드되지 않던 호환성 문제를 수정했습니다.
- 패키지 초기화와 명령 등록을 동기 UI-thread 경로로 정리해서, 오래된 Visual Studio 2022 셸에서도 로드될 수 있게 조정했습니다.
- VSIX 안에 따로 넣었던 `Microsoft.VisualStudio.Threading` 및 관련 런타임 DLL을 제거했습니다.
- 결과적으로 새 설치본은 Visual Studio가 기본 제공하는 셸/런타임 구성만 사용합니다.
...
2. 이 릴리스에서 `CppClassHereVsix-1.3.6.vsix`를 다운로드합니다.
3. `.vsix` 설치 파일을 실행합니다.
