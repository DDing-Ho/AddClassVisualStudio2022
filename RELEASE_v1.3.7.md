...
- 패키지 기반을 `AsyncPackage`에서 일반 `Package`로 변경했습니다.
- `JoinableTaskFactory`, `GetServiceAsync`, `DisposalToken` 같은 비동기 셸 API 의존을 제거했습니다.
- 명령 등록과 초기화를 동기 UI-thread 경로로 단순화해서, 일부 로컬 환경에서 발생하던 셸 API 버전 충돌 가능성을 낮췄습니다.
- VSIX에는 우리 확장 DLL과 언어 팩만 포함되며, `Microsoft.VisualStudio.Threading` 같은 Visual Studio 런타임 DLL은 더 이상 포함하지 않습니다.
...
2. 이 릴리스에서 `CppClassHereVsix-1.3.7.vsix`를 다운로드합니다.
3. `.vsix` 설치 파일을 실행합니다.
