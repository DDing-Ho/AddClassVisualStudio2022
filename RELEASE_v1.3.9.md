# CppClassHereVsix v1.3.9

`CppClassHereVsix`는 Visual Studio 2022에서 C++ 클래스를 더 편하게 만들 수 있게 해주는 확장 프로그램입니다.

기본 `추가 -> 클래스` 기능은 솔루션 탐색기에서 폴더나 필터를 우클릭해도 실제 `.h`, `.cpp` 파일이 프로젝트 루트에 생성되는 경우가 많습니다. 이 확장은 그 불편함을 줄이기 위해 `Add C++ Class Here...` 메뉴를 추가하고, 사용자가 선택한 위치를 기준으로 파일을 만들고 프로젝트에 바로 추가해줍니다.

## v1.3.9 변경 사항

- `v1.3.8`에서 추가한 DPI 대응을 다시 보정한 후속 패치 릴리스입니다.
- Visual Studio owner 창에서 가져온 초기 DPI 값을 다이얼로그 핸들 생성 시점에 다시 덮어쓰지 않도록 수정했습니다.
- 다이얼로그가 실제로 표시된 뒤 현재 DPI를 한 번 더 확인해서, 첫 표시 시점에도 배율이 어긋난 경우 즉시 레이아웃을 다시 맞추도록 조정했습니다.
- 결과적으로 일부 환경에서 남아 있던 초기 표시 크기 오차와 간격 불일치 가능성을 줄였습니다.

## 설치하면 무엇이 추가되나요?

확장을 설치하면 아래 위치에서 `Add C++ Class Here...` 메뉴를 사용할 수 있습니다.

- 솔루션 탐색기에서 프로젝트, 폴더, 필터를 우클릭했을 때의 `추가` 메뉴
- 상단 `도구` 메뉴

## 릴리스 자산

- `Installer\CppClassHereVsix-1.3.9.vsix`

## 설치 방법

1. Visual Studio 2022를 종료합니다.
2. 이 릴리스에서 `CppClassHereVsix-1.3.9.vsix`를 다운로드합니다.
3. `.vsix` 설치 파일을 실행합니다.
4. 설치가 끝나면 Visual Studio를 다시 실행합니다.

## 지원 대상

- Visual Studio 2022
- `vcxproj` 기반 C++ 프로젝트
