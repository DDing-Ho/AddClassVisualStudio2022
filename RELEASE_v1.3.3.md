...
- Visual Studio 시작 시 패키지를 자동 로드하지 않도록 변경했습니다.
- 이제 확장은 `Add C++ Class Here...` 명령을 실제로 사용할 때 로드됩니다.
- 일부 환경에서 Visual Studio 시작 직후 `VsPackage` 로드 오류가 뜨던 문제를 줄이기 위한 수정입니다.
...
- `Installer\CppClassHereVsix-1.3.3.vsix`

## 설치 방법

1. Visual Studio 2022를 종료합니다.
2. 이 릴리스에서 `CppClassHereVsix-1.3.3.vsix`를 다운로드합니다.
3. `.vsix` 설치 파일을 실행합니다.
